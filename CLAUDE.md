...
No mobile (`≤860px`), a barra lateral inteira (marca, seletor de espaço, nav, rodapé) vira uma gaveta (`.side.open`, deslizando de `translateX(-100%)`) aberta por um botão hambúrguer fixo no topo (`.mbar`/`data-action="toggleMobileNav"`) — troca de espaço funciona igual ao desktop dentro da gaveta. A gaveta fecha sozinha ao navegar (`case 'nav'`/`'setSetor'`), ao clicar no backdrop (`#mbarBackdrop`) ou com Esc. Antes disso a nav virava uma fileira horizontal de ~10 ícones minúsculos no rodapé (ilegível com tantas abas) e o seletor de espaço ficava escondido — não usar mais esse padrão de "barra horizontal" se a nav crescer ainda mais.
